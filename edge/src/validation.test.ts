...
import {validChasePreset,validChasePresetName,validIngest,validProfile} from "./validation";
...
describe("공개 입력 검증",()=>{
 ...
 expect(validChasePreset({id:"preset_123",name:"길드 추월",periodDays:1,ocids:[],sortKey:"none",sortDirection:"asc"})).toBe(false);
  expect(validChasePresetName({name:"수정한 이름"})).toBe(true);
  expect(validChasePresetName({name:"줄바꿈\n이름"})).toBe(false);
  expect(validChasePresetName({name:"이름",extra:true})).toBe(false);
 });
