...
// Android 앱 내장 빌드: 대시보드는 /, 직접 조회 엔진은 /direct/에 두고 같은 앱 출처를 씁니다.
const app=process.env.WEB_APP==="1"&&!adHost,APP_ORIGIN="http://tauri.localhost";
...
const {dashboardOrigin,directOrigin}=app?{dashboardOrigin:APP_ORIGIN,directOrigin:APP_ORIGIN}:webOrigins(env);
...
const monetizationEnabled=!app&&String(env.WEB_MONETIZATION_ENABLED||"0")==="1";
...
return {
 ...
 define:{"__EXP_TABLE__":JSON.stringify(table),"__DASHBOARD_ORIGIN__":JSON.stringify(dashboardOrigin),"__DIRECT_ORIGIN__":JSON.stringify(directOrigin),"__APP__":JSON.stringify(app),"__API_ORIGIN__":JSON.stringify(app?"https://guildfollow.com":""),"__DIRECT_PATH__":JSON.stringify(app?"/direct":""),"__MONETIZATION__":JSON.stringify(monetization),"__ADVERTICA__":JSON.stringify(adHost?advertica:{})},
 resolve:{alias:{"/src":path.resolve("src")}},
 base:app&&direct?"/direct/":"/",
 build:{outDir:app?path.resolve("web-dist","app",direct?"direct":""):path.resolve("web-dist",target),emptyOutDir:true},
