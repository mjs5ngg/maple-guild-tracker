...
`release/guildmate-follow-android-arm64-v0.2.12.apk`를 Android 기기로 옮겨 실행합니다. 브라우저나 파일 관리자가 요청하면 해당 앱에만 `알 수 없는 앱 설치` 권한을 허용합니다. Android 7.0 이상 ARM64 기기를 지원합니다.
...
알림 권한을 허용하면 대표 캐릭터를 제외한 즐겨찾기 캐릭터의 경험치 증가를 약 15분 주기로 확인합니다. 첫 확인은 기준점만 저장하며, 경험치 증가가 시작될 때 한 번 알리고 증가가 멈출 때까지 같은 캐릭터의 알림을 반복하지 않습니다. 권한이 꺼져 있거나 감시 작업이 실패·정체되면 앱 사용 중 최대 30분에 한 번 설정 안내창을 표시합니다. Android 절전 정책에 따라 실제 확인 시각은 늦어질 수 있습니다.
