...
`release/guildmate-follow-android-arm64-v0.3.13.apk`를 Android 기기로 옮겨 실행합니다. 브라우저나 파일 관리자가 요청하면 해당 앱에만 `알 수 없는 앱 설치` 권한을 허용합니다. Android 7.0 이상 ARM64 기기를 지원합니다.

최근 7일·30일은 오늘을 포함합니다. 자정 경계에서 전일 종료 자료가 없으면 수집 시각을 기준으로 오늘 획득량을 추정하고, 오전 2시 이후 전일 자료가 수집되면 보정합니다. 앱 설정의 경험치 날짜 안내에서 오차와 보정 정책을 확인할 수 있습니다. 오늘의 0은 성장 평균에 포함합니다.
...
- 즐겨찾기 순위 대형 위젯은 대표 캐릭터를 포함한 즐겨찾기의 오늘 경험치 순위를 고정 높이 행으로 표시하며 목록이 많으면 스크롤할 수 있습니다.
- 대표 캐릭터 주간 성장 3×2 위젯은 가로 5칸까지 늘릴 수 있으며 최근 7일의 날짜·레벨·경험치율·획득량과 일평균·남은 경험치·예상 레벨업 날짜를 표시합니다.
- 대표 캐릭터 성장 2×2 위젯은 같은 정보를 정사각형으로 크게 표시합니다.

위젯은 앱에서 동기화된 로컬 기록을 표시합니다. 앱을 열어 동기화하면 위젯과 캐릭터 이미지도 즉시 갱신됩니다.
