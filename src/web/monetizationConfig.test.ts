...
import {hasDisplayAds,normalizeMonetization,shouldLoadAds} from "./monetizationConfig";
...
describe("공개 웹 광고 설정",()=>{
 ...
 it("명시적으로 허용한 경우에만 외부 광고 로드를 허용",()=>{const config=normalizeMonetization({ads:{mobileBottom:true},adHostOrigin:"https://ads.example.com"});expect(shouldLoadAds(config,null)).toBe(false);expect(shouldLoadAds(config,"declined")).toBe(false);expect(shouldLoadAds(config,"accepted")).toBe(true);});
});
