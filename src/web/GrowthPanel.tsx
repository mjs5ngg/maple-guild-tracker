...
export default function GrowthPanel({character,days,onDays}:{character?:Snapshot;days:number;onDays?:(value:1|7|30)=>void}){
 ...
 return <section className="surface growth-panel" id="growth" aria-labelledby="growth-title">
  <div className="section-heading"><div><span className="section-kicker">EXP PROGRESS</span><h2 id="growth-title">경험치율 흐름</h2>{character&&<div className="growth-character"><b>{character.basic.character_name}</b><small>Lv.{character.basic.character_level} · 오늘 포함 {days}일</small></div>}</div>{onDays?<div className="period-control" aria-label="그래프 기간">{([7,30] as const).map(value=><button key={value} aria-pressed={days===value} onClick={()=>onDays(value)}>{value}일</button>)}</div>:null}</div>
  {character?<div className="chart-area"><ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={1}><LineChart data={points} margin={{top:16,right:8,bottom:18,left:-14}}><CartesianGrid stroke="var(--border)" vertical={false}/><XAxis dataKey="date" tick={{fill:"var(--text-muted)",fontSize:12}} tickMargin={18} height={44} axisLine={false} tickLine={false}/><YAxis domain={[0,100]} ticks={[0,25,50,75,100]} unit="%" tick={{fill:"var(--text-muted)",fontSize:12}} axisLine={false} tickLine={false}/><Tooltip content={<GrowthTooltip/>}/><Line type="monotone" dataKey="percent" stroke="var(--accent)" strokeWidth={3} connectNulls={false} dot={<LevelDot/>} activeDot={{r:6}}/></LineChart></ResponsiveContainer></div>:<div className="empty-state"><b>성장 기록을 기다리고 있어요.</b><span>대표캐릭터를 저장하고 첫 수집이 끝나면 표시됩니다.</span></div>}
 </section>;
}
