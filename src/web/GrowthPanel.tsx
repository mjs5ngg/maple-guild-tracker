...
export default function GrowthPanel({character,days,onDays}:{character?:Snapshot;days:number;onDays?:(value:1|7|30)=>void}){
 ...
 return <section className="surface growth-panel" id="growth" aria-labelledby="growth-title">
  <div className="section-heading"><div><span className="section-kicker">EXP PROGRESS</span><h2 id="growth-title">경험치율 흐름</h2>{character&&<div className="growth-character"><Avatar character={character}/><span><b>{character.basic.character_name}</b><small>Lv.{character.basic.character_level} · 오늘 포함 {days}일</small></span></div>}</div>{onDays?<div className="period-control" aria-label="그래프 기간">{([7,30] as const).map(value=><button key={value} aria-pressed={days===value} onClick={()=>onDays(value)}>{value}일</button>)}</div>:null}</div>
