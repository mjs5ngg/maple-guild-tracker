...
function App(){
 const [key,setKey]=useState(()=>localStorage.getItem(KEY)||""),[remember,setRemember]=useState(()=>!!localStorage.getItem(KEY));
 const [kind,setKind]=useState(localStorage.getItem(KIND)||"development"),[primary,setPrimary]=useState(""),[names,setNames]=useState<string[]>([]);
 ...
 const until=Number(localStorage.getItem("maple-next-refresh")||0);
 if(Date.now()<until){setMessage("연속 조회 방지를 위해 잠시 기다려 주세요.");return;}
 setBusy(true);setCount(0);
 try{
 if(remember)localStorage.setItem(KEY,key.trim());else localStorage.removeItem(KEY);
 ...
 finally{localStorage.setItem("maple-next-refresh",String(Date.now()+60000));setBusy(false);}
 }
 ...
 <label className="check"><input type="checkbox" checked={remember} onChange={e=>{setRemember(e.target.checked);if(!e.target.checked)localStorage.removeItem(KEY);}}/>이 브라우저에 키 기억하기</label><p className="muted">공용 기기에서는 저장하지 마세요. 브라우저 저장소와 확장 프로그램을 통한 노출 가능성은 남습니다.</p>
 <div className="actions"><button disabled={busy||!primary||!key.trim()}>{busy?count+"명 조회 중…":"최신 정보 조회"}</button><button type="button" onClick={()=>{localStorage.removeItem(KEY);setKey("");setRemember(false);}}>저장된 키 삭제</button></div></form><p role="status">{message}</p><a href="https://openapi.nexon.com/ko/guide/prepare-in-advance/" target="_blank" rel="noreferrer">API 발급 안내</a></section><footer>Data based on NEXON Open API</footer></main>;
}
