...
class DirectEngine{
 ...
 private importAndroid(rows:Snapshot[],guildKey:string){if(!window.AndroidDirect)return;try{// 위젯은 최근 7일만 쓰고 네이티브는 31일까지 받으므로 최신 31일만 보냅니다.
 const recent=rows.map(row=>({...row,history:[...row.history].sort((a,b)=>a.date.localeCompare(b.date)).slice(-31)}));window.AndroidDirect.importSnapshots(JSON.stringify({primary:this.primary,favorites:this.favorites,guildKey,rows:recent}));}catch{/* Android 네이티브 저장 실패는 웹 직접 조회를 막지 않습니다. */}}
...
function Setup(){
 ...
 async function pasteKey(){try{if(!navigator.clipboard?.readText)throw new Error("unsupported");const value=(await navigator.clipboard.readText()).trim();if(!value){setMessage("클립보드에 붙여넣을 서비스 키가 없습니다.");return;}setKey(value);setMessage("클립보드의 서비스 키를 입력했습니다.");}catch{setMessage("클립보드 접근이 차단되었습니다. 입력란을 길게 누르거나 직접 붙여넣어 주세요.");}}
 const serviceUrl=APP_MODE?PUBLIC_SITE:__DASHBOARD_ORIGIN__,copyServiceUrl=async()=>{try{await navigator.clipboard.writeText(serviceUrl);setMessage("정식 서비스 주소를 복사했습니다.");}catch{setMessage(`정식 서비스 주소 ${serviceUrl}`);}};
 return <main className="personal-page embedded"><header><span className="section-kicker">PRIVATE DIRECT SYNC</span><h1>서비스 키 직접 조회</h1><p>키와 경험치 기록은 이 기기에만 저장됩니다.</p></header><section className="panel"><label>넥슨 서비스 단계 API 키<div className="key-input-field"><input name="password" type="password" autoComplete="current-password" value={key} onChange={event=>setKey(event.target.value)}/><button type="button" aria-label="클립보드에서 서비스 키 붙여넣기" title="붙여넣기" onClick={()=>void pasteKey()}><ClipboardPaste/></button></div></label><label className="check"><input type="checkbox" checked={confirmed} onChange={event=>setConfirmed(event.target.checked)}/>내 명의의 서비스 단계 키이며 다른 사람과 공유하지 않았습니다.</label><p className="muted">개발키는 지원하지 않습니다. 키 종류를 자동 판별하는 공식 API가 없어 전체 조회 중 반복적인 429가 발생하면 서비스 키 확인을 안내합니다.</p><label>서비스 애플리케이션 등록 URL<div className="copy-field"><input readOnly value={serviceUrl}/><button type="button" onClick={()=>void copyServiceUrl()}>복사</button></div></label><p className="muted">저장소 보호 {persistent===true?"유지 허용":persistent===false?"브라우저 정리 시 삭제될 수 있음":"확인 중"}</p><div className="actions"><button type="button" onClick={()=>void save()}>키 저장 및 자동 조회</button><button type="button" onClick={()=>{writePersonal(KEY,null);writePersonal(CONFIRMED,null);setKey("");setConfirmed(false);notifyControl();setMessage("저장된 키를 삭제했습니다.");}}>저장된 키 삭제</button></div><p role="status">{message}</p><a href="https://openapi.nexon.com/ko/guide/prepare-in-advance/" target="_blank" rel="noreferrer">서비스 키 발급 안내</a><p className="muted">사용자 본인 명의의 키만 입력하고 NEXON 약관을 확인해 주세요. 이 키는 대시보드나 우리 서버로 전달되지 않습니다. 사용자별 서비스 키 사용 방식은 NEXON의 명시적 허용 사례가 아니므로 공식 문의와 병행 운영합니다.</p></section><footer>Data based on NEXON Open API</footer></main>;
}
