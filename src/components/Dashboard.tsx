      ...
      <main className="dashboard-main">
        <header className="topbar">
          ...
          <div className="top-actions">
            ...
            <div className="display-control-wrap"><button className="icon-action" title="화면 크기 조절" onClick={() => setDisplayOpen((value) => !value)}><SlidersHorizontal /></button>{displayOpen && <div className="display-controls"><label><Type />전체 크기 <b>{Math.round(uiScale * 100)}%</b><input type="range" min="0.9" max="1.3" step="0.02" value={uiScale} onChange={(event) => changeUiScale(Number(event.target.value))} /></label><label><Image />캐릭터 이미지 <b>{Math.round(avatarScale * 100)}%</b><input type="range" min="0.95" max="2.25" step="0.05" value={avatarScale} onChange={(event) => changeAvatarScale(Number(event.target.value))} /></label><button onClick={() => { changeUiScale(1.06); changeAvatarScale(1.15); }}>기본값</button></div>}</div>
